...
citation:
  url: https://jaj42.github.io/blog/posts/202508_stanpump/index.html
---
...
Since TCI's first uses in the 1980s @struys_history_2016, significant effort has been invested in developing new PKPD models. Today, these models are available for nearly every drug used in anesthesia. Extensive literature exists on PKPD models and their design, along with various software options for building and fitting these models. NONMEM is the best-known software, but many alternatives are available, including open-source options like nlmixr2 @nlmixr and more recently [OpenPMX](https://github.com/deleveld/OpenPMX).
...
In this closed form solution, the $B$ terms are called hybrid rate constants and the $\lambda$ terms are called exponential decay constants.
You will also find it written as:
...
If we want to solve the ODE system while taking into account the infusion term $J$, the equation becomes more complicated @shafer_algorithms_1992,@bailey_simple_1991. This equation introduces coefficients $c_{ji}$ which are outlined below.
...
The code could be coupled to a syringe pump control library such as [InfuPy](https://github.com/jaj42/infupy) @joachim_jona_infupy_2021 to drive syringe pumps with the calculated rates. Of course this can only be used for research purposes and under no circumstances on real patients. See <https://www.demed.be/Rugloop%20&%20TCI%20news.htm#Background> for more information on this topic.
