...
citation:
  url: https://jaj42.github.io/blog/posts/202602_udf/
---

## STANPUMP again

In my [previous blog post](../202508_stanpump/), I took a deep dive into STANPUMP's source code to understand the underlying logic for Target-Controlled Infusion (TCI) and reimplemented a minimal version in Python. However, we only glimpsed over the actual core algorithm which calculates the required infusion rates to achieve target concentrations.
...
> *What happens if I infuse 1 μg/s for 10 seconds, then stop?*

The UDF is pre-calculated once at the start of the case using these steps:

1. Initialize: Start with empty compartments (all zeros)
...
- Therefore you need 0.3 / 0.19 = 1.58 μg/s for 10 seconds
- This amount to a bolus of 15.7 μg over the first 10 seconds
...
After the peak which occurs exactly at the target effett, the concentration decreases again as the drug  is distributed and metabolized.

![TCI infusion based on a scaled UDF with a single initial bolus](Figure_1.png){#fig-bolus}

## Implementing the TCI Algorithm

If you want to continue infusion after the initial bolus, an iterative algorithm is needed where the infusion rate is recalculated at every time step.
Let's go through the complete TCI algorithm, one step at a time.

### The Pharmacometric Model

We will use the Gepts Sufentanil model again since it is convenient. This code uses effect site concentrations but the same algorithm applies to plasma concentration targets.
...
Charles Minto recently wrote [a comprehensive document](https://www.linkedin.com/feed/update/urn:li:activity:7426118342811213824/) which explains exactly how the eigenvalues are calculated in the STANPUMP code.
...
The TCI algorithm runs every 10 seconds, recalculating the infusion rate based on the current target. These 10 seconds are a convention introduced by STANPUMP but this is a setting that can be modified.
...
The expensive linear algebra happened only **once** in the beginning of the session. The real-time algorithm is just arithmetic and array access which was fast enough to run on a 1990s microcontroller.
...
- **Charles Minto** for the exceptional mathematical derivation document of the eigenvalues (Looking forward to the infusion coefficient derivation!)
