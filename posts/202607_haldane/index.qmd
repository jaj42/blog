...
The two resulting datasets were analysed with NONMEM using First Order (FO) approximation and PyMC using Hamiltonian Monte Carlo (NUTS).
...
The informative fit converges in 21 evaluations towards the true value. The uninformative
...
optimiser settles around the true value in a handful of steps with a gradient
...
![NONMEM optimiser traces. Top: the slope estimate vs iteration. Informative (left) converges towards the true value; uninformative (right) dives to the lower bound. Bottom: the gradient of the objective on the transformed scale.](nonmem_diagnostics.png){#fig-nonmem-diag}
